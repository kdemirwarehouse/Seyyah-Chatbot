...
def api_query():
    if not RAG_AKTIF:
        return jsonify({"error": "RAG motoru aktif değil."}), 503
    ...
    sehir = data.get("sehir") or None
    sinif = data.get("sinif") or None
    # Frontend'den gelen konuşma geçmişi — yoksa boş liste
    ...
    try:
        sonuc = sorgula_ve_yanitla(soru, sehir=sehir, sinif=sinif, gecmis=gecmis)
